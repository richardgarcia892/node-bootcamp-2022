...
const createBookingCheckout = async dataObj => {
  const tour = dataObj.client_reference_id;
  const user = (await User.findOne({ email: dataObj.customer_email })).id;
  const price = dataObj.amount_subtotal / 100;
